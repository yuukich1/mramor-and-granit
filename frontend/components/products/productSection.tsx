...
export function ProductsSection() {
    const { allProducts, loading } = useProducts({ apiLimit: 1000 });
    ...
    const allTags = useMemo(() => {
        ...
        allProducts.forEach(p => p.tags?.forEach(tag => set.add(tag)));
        return Array.from(set);
    }, [allProducts]);


    const indexedProducts = useMemo(() => buildSearchIndex(allProducts), [allProducts]);
    const searchedProducts = useMemo(() => searchIndexedProducts(indexedProducts, query), [indexedProducts, query]);
    const filteredProducts = useMemo(() => filterProducts(searchedProducts, filters), [searchedProducts, filters]);
    ...
    return (
        <section className="py-20 bg-white relative overflow-hidden" id="catalog">
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
                <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="text-center mb-16">
                    ...
                    <p className="text-lg text-neutral-600 max-w-2xl mx-auto drop-shadow-sm">
                        Широкий выбор мемориальных памятников из натурального камня. Возможность создания индивидуального проекта.
                    </p>
                </motion.div>

                <ProductSearch value={query} onChange={setQuery} />
                <ProductFilters filters={filters} categories={categories} allTags={allTags} onChange={setFilters} />
                    ...
                    <>
                        {visibleProducts.length === 0 ? (
                            <div className="text-center py-12">
                                <p className="text-lg text-neutral-600">Ничего не найдено</p>
                            </div>
                        ) : (
                            <>
                                <ProductsGrid products={visibleProducts} categoriesMap={categoriesMap} onSelect={setSelectedProduct} />
                                {loading && (
                                    <div className="mt-8">
            ...
            <ProductModal product={selectedProduct} categoryName={selectedProduct ? categoriesMap[selectedProduct.category_id] : undefined} onClose={() => setSelectedProduct(null)} />
        </section>
    );
}
